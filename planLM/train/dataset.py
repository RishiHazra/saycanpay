...
class Sample:
    ...
    def __init__(self, seed, goal, initial_state, actions, observations):
        self.seed = seed
        self.goal = goal
            ...
            while i < len(lines):
                if "[Seed]" in lines[i]:
                    ...
                    while i < len(lines) and self.completed_action not in lines[i]:
                        ...
                        observation_match = re.search(r'\[Observation] (.*)', lines[i])
                        if action_step_match:
                            actions.append(action_step_match.group(1))
                        if observation_match:
                            obs = observation_match.group(1)
                            observations.append(obs)
                            # remove (act, obs) pairs where action wasn't executed or didn't lead to a state change
                            if obs == '':
                                actions.pop()
                                observations.pop()
                        i += 1
                    ...
                    samples.append(Sample(seed, goal, initial_state, actions, observations))
                i += 1
...
class CanDataset(Dataset):
    ...
    def process(self, can_save_path):
        ...
        for sample in self.all_samples:
            # if len(sample.actions) < 4 and random.random() > 0.3:  # only for blocks
            #     continue
            context = ['[Goal]', sample.goal, '[Initial State]', sample.initial_state]
            for i in range(len(sample.actions)):
                pos_action = sample.actions[i]
                # previous observation for positive action
                pos_prev_obs = sample.observations[i - 1] if i != 0 else sample.initial_state

                # negative action from the same sample
                # i.e. action executed in a different context
                neg_actions = []
                for ind, action in enumerate(sample.actions):
                    # choose a negative sample that is not the same as positive and
                    # doesn't have the same current observation (cannot be executed in the current state)
                    if action != pos_action:
                        if ind != 0:
                            # previous observation for negative action
                            neg_prev_obs = sample.observations[ind - 1]
                            if neg_prev_obs != pos_prev_obs:
                ...
                for _ in range(num_neg_samples):
                    ...
                    while neg_sample_action == pos_action or neg_sample.initial_state == sample.initial_state:
...
class PayDataset(Dataset):
    ...
    def process(self, pay_save_path):
        ...
        for sample in self.all_samples:
            # goal = f'[Goal] {sample.goal}'
            # context = ['[Initial State]', sample.initial_state]
            context = ['[Initial State]', sample.initial_state, '[Goal]', sample.goal]
            for i in range(len(sample.actions)):
                # normalized distance
                # only for babyai
                # dist = np.float32(1 / (len(sample.actions) - i))
                # dist = np.float32(1. - (len(sample.actions) - i - 1) / len(sample.actions))
                gamma = 1.1  # hanoi (gamma=3), babyai (gamma=1.5)
                final_reward = 1
